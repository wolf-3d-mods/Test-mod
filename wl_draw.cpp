...
int weaponscale[NUMWEAPONS-1] = {SPR_KNIFEREADY, SPR_PISTOLREADY,
    SPR_MACHINEGUNREADY, SPR_CHAINREADY, SPR_KNIFE2READY, SPR_PISTOL2READY};
...
void DrawPlayerWeapon (void)
{
    ...
    if (gamestate.weapon != -1)
    {
        ...
        if(gamestate.chosenweapon == wp_knife)
		{
        ...
        if (gamestate.chosenweapon == wp_pistols)
		{
			if (gamestate.weaponframe == 0)
			{
				SimpleScaleShape(viewwidth/2-41,SPR_LPISTOLREADY,viewheight+1);
				SimpleScaleShape(viewwidth/2+41,SPR_RPISTOLREADY,viewheight+1);
			}
			else if (gamestate.weaponframe == 1)
			{
				SimpleScaleShape(viewwidth/2-41,SPR_LPISTOLATK1,viewheight+1);
				SimpleScaleShape(viewwidth/2+41,SPR_RPISTOLATK1,viewheight+1);
			}
			else if (gamestate.weaponframe == 2)
			{
				SimpleScaleShape(viewwidth/2-41,SPR_LPISTOLATK2,viewheight+1);
				SimpleScaleShape(viewwidth/2+41,SPR_RPISTOLATK2,viewheight+1);
			}
			else if (gamestate.weaponframe == 3)
			{
				SimpleScaleShape(viewwidth/2-41,SPR_LPISTOLATK3,viewheight+1);
				SimpleScaleShape(viewwidth/2+41,SPR_RPISTOLATK3,viewheight+1);
			}
			else if (gamestate.weaponframe == 4)
			{
				SimpleScaleShape(viewwidth/2-41,SPR_LPISTOLATK4,viewheight+1);
				SimpleScaleShape(viewwidth/2+41,SPR_RPISTOLATK4,viewheight+1);
			}
		}
        if(gamestate.chosenweapon == wp_knife2)
		{
 	        SimpleScaleShape(viewwidth/2,shapenum,viewheight+1);
		}        
        if(gamestate.chosenweapon == wp_pistol2)
		{
 	        SimpleScaleShape(viewwidth/2,shapenum,viewheight+1);
		}
    }
...
void DrawLevelName (void)
{         
    LatchDrawPicScaledCoord ((screenWidth-scaleFactor*320)/2,(screenHeight-scaleFactor*(STATUSLINES+159))/2,STATUSBAR2PIC);

    WindowY = (screenHeight - scaleFactor*(STATUSLINES+159))/2;
         
    SETFONTCOLOR(0xf,0x0);
    fontnumber=0;  
    PrintY = 16; 
        
    if (gamestate.episode == 0 && gamestate.mapon == 0)
    {
        US_CPrint("Wolf 3D Castle 1");
    }  
    else if (gamestate.episode == 0 && gamestate.mapon == 1)
    {
        US_CPrint("Wolf 3D Castle 2");
    }            
}
...
{
   if (lastmessage != STR_DADDY)
      {
      messagetime = 150; // time for message to display

      strcpy(gamestate.message, lastmessage);
      TextXVar = ((screenWidth-viewwidth)/4)+2;
      TextYVar = ((TextXVar-2)/2)+1;
      if(viewwidth == screenWidth) { TextXVar = 2; TextYVar = 2; }
      }
}   
...
void DrawMessage (void)
{
   ...
   PrintX=TextXVar; PrintY=TextYVar; // position the message
