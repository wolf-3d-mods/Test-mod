...
{ 
  int   i,buttons;
   ...
   if (!gamestate.ammo)      // must use knife with no ammo 
    ...
    if (gamestate.playernum == ch_player2)  
    {    
        if (Keyboard[sc_1] && gamestate.bestweapon >= wp_knife2) 
        { 
          gamestate.weapon = gamestate.chosenweapon = wp_knife2; 
        } 
        if (Keyboard[sc_2] && gamestate.bestweapon >= wp_pistol2)
        { 
        ...
        if (Keyboard[sc_5] && gamestate.bestweapon >= wp_pistols) 
        { 
          gamestate.weapon = gamestate.chosenweapon = wp_pistols;
        }  
     }
     else
     {
...
void DrawLives (void)
{
    ...
    LatchNumber (13,16,2,gamestate.deaths);
}
...
void GiveDeaths (void)
{
    if (gamestate.deaths<99)
        gamestate.deaths++;
    DrawLives ();
}
...
void Cmd_Fire (void)
{
	...
	else if (gamestate.chosenweapon == wp_pistols)
	{
		gamestate.attackcount =
...
void SpawnPlayer2 (int tilex, int tiley, int dir)
{
    player->obclass = player2obj;
    player->active = ac_yes;
    player->tilex = tilex;
    player->tiley = tiley;
    player->areanumber = (byte) *(mapsegs[0]+(player->tiley<<mapshift)+player->tilex);
